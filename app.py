...
def create_chart_display(chart_type, chart_key):
...
if page == "AI Chat":
    ...
    with st.expander("🔍 Debug Info"):
        st.write(f"show_chart: {st.session_state.show_chart}")
        st.write(f"current_chart_type: {st.session_state.current_chart_type}")
        st.write(f"chart_displayed: {st.session_state.chart_displayed}")
    ...
    with col4:
        if st.button("🔄 Clear Charts"):
            # Completely reset all chart state
            st.session_state.show_chart = False
            st.session_state.current_chart_type = None
            st.session_state.chart_displayed = None
            # Force a clean rerun
    ...
    # Chat history display - NO CHARTS HERE BUT TRIGGERS CHART ACTIVATION
    ...
    for message in st.session_state.chat_history:
        if message["role"] == "user":
            ...
            if "CHART_REQUEST:" in message['content']:
                ...
                # CRITICAL FIX: Activate chart in the separate section above
                chart_type = parse_chart_request(message['content'])
                if chart_type:
                    # Trigger chart display by setting session state
                    st.session_state.show_chart = True
                    st.session_state.current_chart_type = chart_type
                    st.session_state.chart_displayed = int(time.time())
            else:
    ...
    # Chart display section - MOVED BELOW conversation to fix timing
    if st.session_state.show_chart and st.session_state.current_chart_type and st.session_state.chart_displayed:
        st.markdown("### 📊 Active Chart")
        chart_key = f"{st.session_state.current_chart_type}_{st.session_state.chart_displayed}"
        create_chart_display(st.session_state.current_chart_type, chart_key)
    ...
    if st.button("Send", type="primary") and user_input:
        ...
        # Rerun to update display and activate any charts
        st.rerun()
    ...
    if st.session_state.show_chart and st.session_state.current_chart_type and st.session_state.chart_displayed:
        st.markdown("### 📊 Active Chart")
        chart_key = f"{st.session_state.current_chart_type}_analytics_{st.session_state.chart_displayed}"
        create_chart_display(st.session_state.current_chart_type, chart_key)
