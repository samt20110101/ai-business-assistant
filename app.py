...
# COMPLETELY FIXED: Chart state management
if 'chart_display_key' not in st.session_state:
    st.session_state.chart_display_key = 0

if 'current_chart_type' not in st.session_state:
    st.session_state.current_chart_type = None

if 'chart_cleared' not in st.session_state:
    st.session_state.chart_cleared = False

# FIXED: Better chart management functions
def activate_chart(chart_type):
    """Activate a new chart and force refresh"""
    st.session_state.current_chart_type = chart_type
    st.session_state.chart_display_key += 1  # Force new render
    st.session_state.chart_cleared = False
    
def clear_all_charts():
    """Completely clear all charts"""
    st.session_state.current_chart_type = None
    st.session_state.chart_display_key += 1  # Force refresh
    st.session_state.chart_cleared = True
...
def render_chart(chart_type):
    ...
    chart_key = f"{chart_type}_{st.session_state.chart_display_key}"
...
if page == "AI Chat":
    ...
    # FIXED: Dynamic chart display section
    st.markdown("### 📊 Chart Display")
    
    # Show chart or cleared message
    if st.session_state.chart_cleared:
        st.markdown("""
        <div class="chart-cleared">
            <h4>📭 Charts Cleared</h4>
            <p>Ask me to create a new chart or use the quick buttons above!</p>
        </div>
        """, unsafe_allow_html=True)
    elif st.session_state.current_chart_type:
        render_chart(st.session_state.current_chart_type)
    else:
        st.info("💡 No active charts. Ask me to create one!")
    ...
    for message in st.session_state.chat_history:
        if message["role"] == "user":
            ...
            # Check for chart requests in the message
            if "CHART_REQUEST:" in message['content']:
                ...
                if chart_type and chart_type != st.session_state.current_chart_type:
    ...
    if st.button("Send", type="primary") and user_input:
        ...
        # Check for chart requests and activate immediately
