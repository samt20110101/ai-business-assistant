...
# FIXED: Simple chart state management - NO MORE COMPLEX VARIABLES!
if 'active_chart' not in st.session_state:
    st.session_state.active_chart = None

if 'chart_refresh_id' not in st.session_state:
    st.session_state.chart_refresh_id = 0

# FIXED: Simple chart functions - NO RERUNS!
def activate_chart(chart_type):
    """Activate a chart - simple and bulletproof"""
    st.session_state.active_chart = chart_type
    st.session_state.chart_refresh_id += 1
...
def parse_chart_request(response):
    """Parse chart request from AI response - FIXED MAPPING"""
    for line in response.split('\n'):
        if line.startswith('CHART_REQUEST:'):
            try:
                parts = line.replace('CHART_REQUEST:', '').split('|')
                chart_type = parts[0].strip() if len(parts) > 0 else ""
                data_focus = parts[1].strip() if len(parts) > 1 else ""
                
                # FIXED: Better mapping logic
                print(f"DEBUG: Chart type: '{chart_type}', Data focus: '{data_focus}'")  # Debug
                
                # Map based on BOTH chart type AND data focus
                if 'pie' in chart_type.lower():
                    if 'customer' in data_focus.lower():
                        return 'customer_pie'
                    elif 'expense' in data_focus.lower():
                        return 'expense_pie'
                    else:
                        return 'customer_pie'  # Default pie chart
                        
                elif 'bar' in chart_type.lower():
                    if 'region' in data_focus.lower():
                        return 'regional_bar'
                    else:
                        return 'regional_bar'  # Default bar chart
                        
                elif 'line' in chart_type.lower():
                    if 'profit_margin' in data_focus.lower() or 'margin' in data_focus.lower():
                        return 'profit_margin'
                    else:
                        return 'revenue_trend'  # Default line chart
                else:
                    # Fallback based on data focus only
                    if 'customer' in data_focus.lower():
                        return 'customer_pie'
                    elif 'expense' in data_focus.lower():
                        return 'expense_pie'
                    elif 'profit_margin' in data_focus.lower() or 'margin' in data_focus.lower():
                        return 'profit_margin'
...
def render_chart(chart_type):
    """Render chart based on type with unique key"""
    chart_key = f"{chart_type}_{st.session_state.chart_refresh_id}"
...
if page == "AI Chat":
    ...
    # FIXED: Chart Display Section - ALWAYS VISIBLE, NO COMPLEX LOGIC
    st.markdown("### 📊 Chart Display")
    
    # Show chart status
    if st.session_state.active_chart:
        st.markdown(f'<div class="chart-status">📊 ACTIVE CHART: {st.session_state.active_chart.upper()}</div>', unsafe_allow_html=True)
        render_chart(st.session_state.active_chart)
    else:
        st.markdown("""
        <div class="chart-cleared">
            <h4>📭 No Active Charts</h4>
            <p>Ask me to create a chart or use the quick buttons above!</p>
        </div>
        """, unsafe_allow_html=True)
    
    # FIXED: Process any new chart requests from recent messages
    if len(st.session_state.chat_history) > 0:
        latest_message = st.session_state.chat_history[-1]
        if latest_message["role"] == "assistant" and "CHART_REQUEST:" in latest_message['content']:
            chart_type = parse_chart_request(latest_message['content'])
            if chart_type and chart_type != st.session_state.active_chart:
                activate_chart(chart_type)
                # Force immediate rerun to show correct chart
                st.rerun()
    ...
    if st.session_state.active_chart:
        ...
        st.info("💡 Click a button above to generate a chart!")
