...
# FIXED: Chart state management - simplified and bulletproof
if 'active_chart' not in st.session_state:
    st.session_state.active_chart = None
    
if 'chart_refresh' not in st.session_state:
    st.session_state.chart_refresh = 0
...
def activate_chart(chart_type):
    ...
    st.session_state.chart_refresh += 1
...
def render_chart(chart_type):
    ...
    chart_key = f"{chart_type}_{st.session_state.chart_refresh}"
...
if page == "AI Chat":
    ...
    # FIXED: Chart display section - ALWAYS show this section
    st.markdown("### 📊 Chart Display")
    
    # Show current chart status
    if st.session_state.active_chart:
        st.info(f"📊 Active Chart: {st.session_state.active_chart}")
    ...
    for i, message in enumerate(st.session_state.chat_history):
        if message["role"] == "user":
            ...
            if "CHART_REQUEST:" in message['content']:
                ...
                # FIXED: Process chart request WITHOUT rerun (avoid infinite loop)
                chart_type = parse_chart_request(message['content'])
                if chart_type:
                    # Only activate if this is the most recent message AND not already active
                    if i == len(st.session_state.chat_history) - 1 and st.session_state.active_chart != chart_type:
